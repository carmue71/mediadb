...
class EpisodeController extends FileContainterController
{
    ...
    public function showMovies(){
        $id = $_GET['id'];
        $movies = $this->fileRepository->findFilesForEpisode($id, 3);
        if ( count($movies) > 0 ){
        ...
        foreach ($candidates as $candidate) {
            ...
            if ( $id_actor > -1 ){
                if ( !$this->actorRepository->isAlreadyLinked($id_actor, $id) ){
                    $this->actorRepository->linkActorToEpisode($id_actor, $id, "autogenerated by MediaDB");
    ...
    protected function render($view, $params)
    {
        ...
        switch ($view) {
            ...
            case 'showmovie':
                ...
                $bodymodifier = $this->getBodymodifier($ms);
                include VIEWPATH.'episodes/episode_movies.php';
                break;
            case 'showfiles':
                $this->updatePageNumbers();
                $ms = $params['entry'];
